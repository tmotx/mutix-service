...
function scrollEvent() {
  ...
  if (document.body.scrollTop > 600 || document.documentElement.scrollTop > 600) {
    header.style.backgroundColor = 'rgba(0, 0, 0, 1.0)';
  } else {
...
function resizeEvent () {
  /*
  if (getWindowWidth() >768) {
    stopEquipmentSpotlight();
  } else {
    stopEquipmentSpotlight();
    playEquipmentSpotlight();
  }*/
...
resizeEvent ();
showInfo('PACKAGE');
showInfo('PACKAGE', 'mobile');
showPriceMobile('L')


/*
var EQUIPMENT_TIMER = null;
var EQUIPMENT_OBJ = document.getElementById('equipment');
var CURRENT_EQUIPMENT_IMAGES_POINTER = 0;
var EQUIPMENT_IMAGES = equipment.getElementsByTagName("img");
*/

/*
function equipmentSpotlight(fileName, tar) {
  if (getWindowWidth() >768) {
    tar.setAttribute('src', './images/equipment/' + fileName);
  }
}

function playEquipmentSpotlight(){
 
  EQUIPMENT_TIMER = setInterval(function(){
    var curTar = EQUIPMENT_IMAGES[CURRENT_EQUIPMENT_IMAGES_POINTER];
    var fileName = curTar.getAttribute('src');
    fileName = fileName.substring(0, fileName.length -5);
    fileName += '2.jpg';
    EQUIPMENT_IMAGES[CURRENT_EQUIPMENT_IMAGES_POINTER].setAttribute('src', fileName);

    if (CURRENT_EQUIPMENT_IMAGES_POINTER == 0) {
      curTar = EQUIPMENT_IMAGES[EQUIPMENT_IMAGES.length - 1];
    } else {
      curTar = EQUIPMENT_IMAGES[CURRENT_EQUIPMENT_IMAGES_POINTER - 1];
    }
    fileName = curTar.getAttribute('src');
    fileName = fileName.substring(0, fileName.length -5);
    fileName += '1.jpg';
    curTar.setAttribute('src', fileName);
    if (CURRENT_EQUIPMENT_IMAGES_POINTER >= EQUIPMENT_IMAGES.length -1) {
      CURRENT_EQUIPMENT_IMAGES_POINTER = 0;
    } else {
      CURRENT_EQUIPMENT_IMAGES_POINTER ++;
    }
  }, 3000);
  
}

function stopEquipmentSpotlight() {
  clearInterval(EQUIPMENT_TIMER);
}
*/